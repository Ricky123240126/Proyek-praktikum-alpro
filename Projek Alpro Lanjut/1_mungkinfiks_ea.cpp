...
void bacaDataDariFile()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        fscanf(file, "%s %s %s %d", nama, pass1, pass2, &saldo);

        dataPengguna.nama[i] = nama;
        dataPengguna.password[i][0] = pass1;
        dataPengguna.password[i][1] = pass2;
        dataPengguna.saldo[i] = saldo;
    }
...
//     if (file == NULL)
//     {
//         cout << "File transaksi.txt tidak ditemukan!!!" << endl;
//         return;
//     }
//     char baris[256];
//     cout << "\n========LOG TRANSAKSI========\n"
...
int main()
{
    ...
    while (batas < 3 && !loginSukses)
    {
        ...
        if (!loginSukses)
        {
            ...
            if (batas < 3)
            {
                cout << "Password salah. Sisa percobaan login Anda: " << 3 - batas << endl;
            }
            else
            {
